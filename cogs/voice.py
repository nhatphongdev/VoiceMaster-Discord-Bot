...
class voice(commands.Cog):
    ...
    async def help(self, ctx):
        ...
        embed.set_footer(text='Bot code by fin.09')
    ...
    async def setup(self, ctx):
        ...
        id = ctx.author.id
        if ctx.author.id == ctx.guild.owner_id or ctx.author.id == 860393691424489493:
            def check(m):
    ...
    async def setlimit(self, ctx, num):
        conn = sqlite3.connect('voice.db')
        c = conn.cursor()
        if ctx.author.id == ctx.guild.owner.id or ctx.author.id == 860393691424489493:
